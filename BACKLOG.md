...
## SPECIFIED: VERTICAL CONSTRUCTION (S3) — upper floors + underground cellars (Ken's directive 2026-07-12 ~16:00)

**Goal (Ken):** settlers build 2nd/3rd/4th-layer floors and rooms above ground, and cellars below ground.

**RECONCILE — what exists:**
- Mod: HouseBuilder LayoutV2 is single-level (2D grid at home y). CellarBuilder v1 = hillside HORIZONTAL dig at home level only (its own fallback line: "needs stairs support (v2)"). WorldSense already maps 16 height levels + counts `tower-cols` and `cellar-capable` columns (42k on Dolgellau). Blueprint placement path takes Vec3Int (y included) — untested above/below home level.
- Game (decompile): stairs are a real building type — `StairsComponentBlueprint` (BuildingType.Stairs, `IsStairWall` variant), `BuildingsManagerMain.CanPlaceStairs(blueprint, gridPositions)` placement gate, `LadderComponentManager.StairsConstructed()` tracks vertical links. `StabilityManager` keeps a per-voxel stability byte grid ("basic_stability" MaxStability), saved per node — unsupported spans collapse. Dig pipeline (DigMarkerResourceManager → CreateDigJobs → DigGoal) already proven by CellarBuilder v1.

**DESIGN — three slices, each independently validatable in game:**
- **V1 STAIRS PRIMITIVE (unlocks everything).** Resolve stair blueprint ids from the buildings repository; place via the existing blueprint path; verify `CanPlaceStairs` semantics (front position, landing cells). Validate: designate stairs from ground to the longhouse roof level; EYES ON settlers building then CLIMBING them. Exit gate: a settler pathing across levels.
- **V2 SECOND STOREY.** LayoutV2 grid goes 3D: `g[level][x,z]`. Ground floor unchanged. Upper level = floor tiles across the footprint (every tile within support distance of a load-bearing wall below — query StabilityManager before commit), perimeter walls, interior partitions, stairwell void where V1's stairs land, roof relocated to the top level. Build-order choreography matters (the game builds what it can reach): ground walls done → upper floor tiles → stairs → upper walls → roof. Architect prompt gains `floors` + per-floor programs ("ground: hall,workshop / upper: dorm,dorm"). Batch caps stay (24/pass).
- **V3 TRUE CELLARS.** Use WorldSense cellar-capable columns: pick the column set under/beside the pantry; dig markers at y-1 (3x3 + stair shaft), stairs DOWN into the void (verify stair placement against dug faces — IsStairWall variant may be the underground one), then floor + door + a food-filtered stockpile zone underground. Exit gate: food items visibly stored in the cellar.
- **Cross-cutting:** stability guard before every above/below-ground cell commit; swamp synergy — raised floors on posts are the eventual answer to marsh ground (bank as V4 candidate: stilt platforms); CellIsDry gate applies at ground level only.

**Order: V1 → V2 → V3** (stairs gate both). Each slice: PLAN→BASELINE→IMPLEMENT→VALIDATE in game→CLOSE per workflow.

## CORRECTION + BISECT: "memory wall" was a MISREAD — freeze is deterministic in the mod tick (2026-07-12 ~16:20)

Hang #3 reproduced with 7.7GB free RAM → memory exonerated (hang #1's WatchDog line was coincident, hang #2's "paging" was the OS trimming an already-hung process). TRUE pattern [REPRODUCED 3/3]: every hang's final mod-log line is `[ColonyBuilder] food: hunt+N forage+M`; the freeze is inside ColonyBuilder's tick between the food log and the cellar log, and `CellarBuilder.DigCellarNear(home, radius=60)` NEVER RETURNED once (LastResult stayed "(idle)" in all three boots — it's set on every exit path). Suspect [HYPOTHESIS, strongly localized]: the ~14k-cell reflection GetNode ring-scan on a map whose water sim is hot (86% water) — possible main-thread/water-thread deadlock inside a map query. 4.8GB minidump of hang #3 banked at scratchpad\gm_hang.dmp for deep analysis if needed.
**HARDENING DEPLOYED:** DigCellarNear now logs scan entry, logs progress every 10 rings, and runs under a 50ms-per-tick time budget with ring-resume (a freeze inside the scan becomes visible + bounded). Bisect run in flight: "scan begin" then silence = confirmed in-scan (next step: disable cellar on >50%-water maps); scan completes but freeze persists = suspect is elsewhere in lines 350-500 of ColonyBuilder.

## FREEZE CULPRIT NAMED [REPRODUCED 4/4]: HouseBuilder site search — fix deployed (2026-07-12 ~16:35)

Breadcrumb instrumentation (validation/tick_phase.txt, phase written before every subsystem call) caught it on the first try: **last phase before the 4th freeze = `house-step`** — `HouseBuilder.Plan()`'s spiral site search. Mechanism: ~360k reflected map queries per pass (radius 2..18 → 37² origins × 12×11 footprint × CanPlaceWallAt+CellIsDry), on the MAIN THREAD, re-run EVERY TICK because failure was never cached (marsh map → always fails) — each pass raced the hot water-sim thread until a map query deadlocked. Intermittency explained: it's a race, and the search only runs when the housing director wants a NEXT building (queue empty + graduation pressure). CellarBuilder exonerated by its entry log never firing (its scan never got reached).
**FIX DEPLOYED (same medicine as cellar):** search entry/complete logs, 50ms/tick budget with radius-resume, and a 5-min cooldown after a failed full pass. Validation soak (25 min post-live) in flight. NOTE: the REAL cure for "search always fails on marsh" is the region-scale village-plot siting in the S3/siting spec — this fix makes failure CHEAP and SAFE, not successful.

## ✅ VERIFIED: FREEZE FIX HOLDS — 25-min stable soak, longest live stretch of the day (2026-07-12 16:52)

Soak evidence: WORLD LIVE 16:26:54 → SURVIVED 25 MIN (watch exit criterion). The site search now runs in visible budgeted slices (log: "site search begin 12x11 (radius 7..18)" → "paused at radius 8/18" → next tick resumes; heartbeats flow between slices). The freeze class is closed: unbounded main-thread map scans were the whole story — HouseBuilder siting (4/4 breadcrumb) with CellarBuilder's identical pattern pre-hardened as a bonus.
Also VERIFIED this soak: **SaveGuard end to end with FILE evidence** — flag save consumed, periodic banks every 8 min (log lines 16:32/16:41/16:49) and real saves on disk (VillageSaves\Dolgellau\Autosave-11.sav 16:41, Autosave-12.sav 16:49, 2MB each + .gmevents + .meta). Max loss window on any future crash: 8 minutes. Colony state: pop 4, food crisis active (reactor hunting/foraging: session hunt=1 forage=18), plan adopted, longhouse standing.
**Suggested commit title:** `fix(stability): time-budget all main-thread map scans (house siting + cellar); breadcrumb tracer; SaveGuard verified with on-disk banks`

## COHERENCE FIXES (Ken eyes-on: starving beside corpses; corpse pile INSIDE the house) — deployed (2026-07-12 ~17:30)

Ken's screenshot: settlers idle/starving, animal corpses on the stockpile, and the corpse stockpile technically INSIDE the longhouse (house was sited over the colony's existing zones; "farthest" refuse zone was only d=7 — also at the house). Three root causes, three fixes:
1. **Missing survival link — BUTCHERING TABLE.** Corpses are not food until butchered (production id `meat`, ground-truthed in production_ids.txt); the campfire's `meal` order sat ingredient-less at stored-nutrition-0. ColonyBuilder Priority 2.5: census-gated butchering_table placement + `meat` production queue. Carcasses confirmed class=WASTE in resource_taxonomy.txt (deer/boar/cow/… ×20).
2. **Zoner inside-house override.** Any zone whose Start falls in the house footprint (new HouseBuilder.FootprintContains) is ALWAYS pantry (food only — a pantry indoors is coherent, corpses aren't); REFUSE goes to the farthest zone OUTSIDE the footprint; if none exists, loud WARNING instead of a silent corpse parlour.
3. **Siting must not squat zones.** StockpileZoner.GetZoneRects() (once per pass — no reflection in hot loops, per today's freeze lesson) + CellInRects checks added to BOTH placement paths: HouseBuilder spiral AND VillageLayout.TryGetSlot (the slot path is the one that placed the house over the zones).
Validation soak in flight: butcher placed → meat queued → nutrition off zero; zoner override lines; 25-min stability.

## FREEZE #5 (17:21:47, breadcrumb house-step DURING spiral cooldown) → cooldown now gates ALL siting (2026-07-12 ~17:45)

The 5-min cooldown only guarded the spiral; VillageLayout.TryGetSlot's ~2k reflected map queries still ran EVERY housing tick and hit the same water-sim race. Lesson sharpened: **the deadlock strikes on ANY main-thread map query — reducing volume only reduces odds; the only safe state is NOT querying.** Hotfix deployed: the cooldown check moved BEFORE the slot path — a failed siting pass silences every map query (slots + spiral) for 5 minutes. Butcher/zoner fixes ride the same build. 30-min validation soak in flight.
Escalation path if a freeze survives even this: WorldSense's boot scan reads the whole map safely ONCE — migrate siting checks onto its cached snapshot (zero live map queries during gameplay); dump analysis (scratchpad\gm_hang.dmp) can confirm the exact lock if needed.

## FREEZE #6 (after zoner line, breadcrumb still at butcher-place = uninstrumented region) → FULL tick breadcrumb coverage deployed (2026-07-12 ~18:00)

Freeze #6 landed in the tick's pre-food region (census/alerts/director/diagnostics/equip/weapons/plan/zoner/events/wood/food) which had no phase markers. Also that boot PROVED the butcher fix fires: `butchering_table committed at (95,5,151)` on the first eligible tick — but blueprint diagnostics immediately flagged it `UNREACHABLE NO-RESOURCES` (placement quality: TryPlaceBuildingNear doesn't check reachability/materials — open follow-up). Deployed: Phase() markers now bracket EVERY subsystem in ColonyBuilder.Tick (~25 phases; the whole loop is covered — the next freeze names its exact call). 35-min watch in flight.
**Standing pattern after 6 freezes: every confirmed culprit is a main-thread reflected map scan racing the water sim.** The endgame fix (next session's unit if freezes persist): migrate ALL live map queries (wood/food/siting/placement checks) onto WorldSense/WorldMap cached snapshots — zero live GetNode during gameplay.

## SPECIFIED (recon done, deploy-gated on Ken's play session): marsh survival pair (2026-07-12 ~18:20)

**Ken started a NEW GAME: Hexham, Marsh map ("teeming with fish, land animals scarce"), small, seed 901811147.** All agent input/screen calls STOPPED (operator rule 2 — Ken is driving); passive file-only observer armed. Fresh save = clean first-boot validation of today's whole chain (home → siteplan → stockpile → cookfire → BUTCHER → beds → house → architect) whenever the mod's autonomy is enabled there.

1. **FISHING mystery (recon result):** `JobType.Fishing=0x100000` is ALREADY in JobRouter.CrisisFoodJobs at prio 1, and `FishingGoal.CanStart` only needs `GetObjectCount(FishMapResource)>0` — no designation needed. Yet nobody fished during Dolgellau's crisis. Gate is live-side: goal-selection weight, water-edge pathing (FishingRadius=6), or tool. VALIDATE ON HEXHAM (fish-rich): if crisis fires and no one fishes, log FishingGoal's selection path. Do NOT code blind.
2. **Butcher placement reachability (spec):** TryPlaceBuildingNear checks CanPlace + not-near-stockpile + no-building but never connectivity → 'UNREACHABLE' commit at (95,5,151). Game API found: nodes → Region; `PathfinderUtil+LineOfSightFilters.IsRegionReachable(WalkableModel, startRegion, endRegion)` (api_index 48883). Fix: after CanPlace accepts a cell, require an ADJACENT walkable node whose Region is reachable from the anchor settler's Region; else next cell. Also applies to TryPlaceStockpileNear + farm siting.

## CANON UPGRADE: Ken's player reference guide distilled — several specs CORRECTED (2026-07-12 ~18:45)

Ken supplied the comprehensive v1.1.x player guide (Desktop) as the villagers' competence bar. Distilled to `F:\StarForge\wiki\going-medieval\player-competence-canon.md` (linked in _index). SPEC CORRECTIONS it forces:
1. **Crisis threshold WRONG:** a settler needs ~12 raw food/DAY (a meal = 12 raw). Our crisis fires at nutrition<pop*6 = half a day's need — far too late. Raise to ~pop*12 with malnutrition signals as EARLY warning (guide: staged malnutrition slows work/heal/XP before HP loss).
2. **Cellar V3 spec REWRITTEN by canon:** cold cellar = dig 2+ LEVELS DOWN (not v1's hillside-at-home-level), fully walled (else RAT infestation), beams every ~3 tiles (cave-ins), wicker/grated floor coldest, no-floor perimeter. **MARSH: cellars FLOOD in spring/autumn — Hexham cellars need full wall layer + mesh drainage or a well, OR store food in an elevated walled granary instead.**
3. **V2 second-storey spec gains exact physics:** stability ground=4, −1 per horizontal step, 0=unplaceable, inherited upward free; beams 15 wood any span, wall-to-wall, not on doors/windows, GATED ON ARCHITECTURE RESEARCH (the first tech); roofs need only 2 stable edge points.
4. **ResearchPlanner priority:** Architecture FIRST (gates beams → all vertical work), then Agriculture, Tailoring (flimsy starting clothes carry a mood debuff — a clothes loop is a missing coherence pillar).
5. **New gap list banked in canon note:** defense construction (chokepoint before first raid — raiders arrive within DAYS; raiders attack doors, ignore walls), room-type formation (Library +20% research = table + 2 wall bookshelves), book protection (lost books stall research), seed reservation, preservation chain (smokehouse), recreation diversity (boredom debuff), fishing (marsh staple).
First-year playbook = the mod's target sequence: shelter → research table → Architecture/Agriculture/Tailoring → cold cellar → crops → clothes → chokepoint.

## NEW-GAME FIRST-BOOT REPORT + cross-village staleness bug FOUND & STAGED (2026-07-12 ~19:00)

Passive observer on Ken's fresh village (pop 4: Jonatan Tilly, Girard Townsend, Margaria Watt, William Alder): **the survival chain worked from tick one on a clean save** — home fixed fresh at (63,6,113), stockpile + campfire placed, crisis plan adopted & completed 4/4 ("Create fire for cooking, gather food"), jobs routed by skill, build-pressure boosting the best builder, schedule applied, periodic autosaves banking. **20+ minutes, zero freezes** (time-budgeted scans visibly pausing/resuming at radius ticks). Honest-but-ugly: "beds 0/4 interior FULL" message misleads when NO house exists yet (cosmetic, backlog); butchering table placement pending (priority ladder correct).
**BUG [REPRODUCED live]: cross-village staleness.** New village's telemetry showed Dolgellau's worldmap line VERBATIM (86% water, tower-cols 73, cellar-capable 42377) — WorldMap.LastScanTicks, WorldSense.LastGrid, and HouseSitePlanner.Done/HasSite survive village changes (no Reset), so the leader's siteplan scored the NEW map against DOLGELLAU'S terrain → "NO site fits a 12x12 pad" on unscanned ground. **FIX STAGED (compiled, deploy on next restart): the three caches reset in BuiltState.ResetSession.**
Staged build now carries: crisis threshold pop*12, research order architecture-first, cross-village cache resets.

## THREE COHERENCE PILLARS STAGED (compiled, deploy on next restart) (2026-07-12 ~19:20)

Built per the player-competence canon, offline while Ken plays; all reuse PROVEN machinery:
1. **STARTER-SHACK FALLBACK (Option A, recommended to Ken — awaiting his veto/blessing).** When v2 longhouse siting fails a full pass AND the save has never completed a shelter: site the LEGACY v1 shack (4x7 — small pads exist where 12x11 doesn't) with the same zone/dry/time-budget guards, persist as v1 plan; the EXISTING graduation machinery upgrades v1→architect longhouse later. Loop-guarded on BuiltState.HouseComplete (no shack sprawl after graduation). Fixes tonight's live incoherence: 4 settlers idling through the night, 0 beds, siting cooling down. Shelter budget 80ms + 30s retry (urgent, unlike the 5-min longhouse cooldown).
2. **PRESERVATION CHAIN**: smokehouse placed once butcher exists (census+skill gated, fletcher pattern) + `smoked_meat` production kept queued. Canon: preserve in autumn or starve in winter (Dowsby's death).
3. **CLOTHES LOOP**: sewing_station once shelter complete + `winter_clothes`/`summer_clothes` kept queued (flimsy starting clothes = standing mood debuff). Orders idle harmlessly until flax/wool exists.
Staged build now carries SIX changes: crisis pop*12, research architecture-first, cross-village cache resets, shack fallback, smokehouse, sewing. All compiled code=0. Remaining unbuilt pillars: defense (design unit — needs live iteration), room-type formation, book protection, seed reservation, fishing validation (observation-gated), region siting, placement reachability.

## DEFENSE v1 BUILT & STAGED — palisade ring on the dry approaches (2026-07-12 ~19:40)

The biggest unbuilt canon pillar, implemented per doctrine (first raid within days; raiders attack DOORS, ignore blank walls; AI won't cross water): **DefenseBuilder** — wooden palisade ring at R=14 around home, placed AFTER the survival floor (house complete). Marsh-elegant: WATER RING CELLS ARE SKIPPED (the marsh is a free moat — only dry approaches get walled); exactly ONE gate (first dry ring cell → wood_door — deterministic funnel point); never walls stockpile zones (zone-rect check); 8 walls/tick material drip; 40ms time budget with ring-index resume (freeze-class safe by construction); goes permanently dormant when a full pass places nothing new. Registered in ResetSession. Wired as Priority 4.65. Merlons/walkways/traps = v2 (research-gated).
STAGED BUILD now carries SEVEN changes, all code=0. VALIDATION PLAN on deploy: eyes-on the ring hugging dry ground with one gate; raider funnel observed at next raid; no stockpile/house cell walled.

## GOAL RESET (Ken /goal 2026-07-12 ~20:00): close the gap to the AI-Influence reference docs — RECONCILED GAP MATRIX

End state: doc 11's five scenarios reproducible live + coherent NPC choices. Current maturity vs each system doc (validated-evidence based):
- **01 AI Dialogues ~70%**: LLM dialogues + trust/disclosure DB + personalities + world-events-in-context LIVE. Gaps: lie detection, conflict escalation, trading-via-dialogue, TTS (optional), #23 batching.
- **02 Dynamic World Events ~40%**: P5 slice 1 LIVE (lived events → API → propagation → dialogue context). Gaps: AI-GENERATED original events, rumor gradient (knows-only today), event evolution, personalization (=P5 slice 2, spec exists).
- **03 AI Diplomacy ~5%** ← BIGGEST GAP × HIGHEST LEVERAGE (4 of 5 scenarios): game gives faction list/alignment/raid attribution only. Needs OUR simulation: relations state machine, wars/alliances/treaties, tribute/reparations, war fatigue/stats, banishment, proclamations (LLM), diplomacy ROUNDS.
- **04 Disease & Plague 0%**: the game has NO disease system (canon-confirmed) — entire simulated layer to build (infection SM, seasonal odds, spread via visitors, quarantine, infirmary tiers, herbal prevention, combat debuffs).
- **05 NPC Memory ~70%**: per-NPC profiles/histories/trust LIVE + merge machinery. Gaps: NPC↔NPC relationship tracking, evolving descriptions.
- **06 Romance & Marriage 0%**: courtship SM over dialogue+memory, NPC initiative, betrothal/marriage, decay.
- **07 Settlement Combat ~10%**: game raids + event reads + DefenseBuilder (staged). Gaps: AI combat direction (aggressor/defenders/panic/companion stances/interventions/transitions/aftermath).
- **08 Death History ~80% (VALIDATED live — Giles/Dowsby)**: gaps: 50+ interaction gate, polished display, opt-out.
- **09 AI Actions ~30%**: P3 order queue LIVE. Gaps: order vocabulary (follow/goto/patrol/hold/band/attack/besiege/raid — needs validated pathing control), issue-via-dialogue, reload persistence proof.
- **10 Additional ~25%**: worldsense/worldmap LIVE. Gaps: NPC initiative, visit history, recruitment detection, non-combatant protection, economic ripples.

**CAMPAIGN SEQUENCE (leverage × buildability):** A) P6 DIPLOMACY (substrate = pure dashboard Python — offline-testable NOW, no deploy window!) → B) Events slice 2 → C) AI Actions order vocabulary → D) Disease layer → E) Romance → F) Combat direction. Continuous: dialogue upgrades, staged-7 validation at deploy window, colony coherence stream.
**NEXT UNIT (SPECIFIED): P6 diplomacy** — design doc for Ken's sign-off + deterministic core (DB schema: faction_relations/wars/treaties/tribute; real-time rounds ticker; war fatigue math; event feed into P5 pipeline; LLM lanes: proclamations + bounded decision menus). Dashboard-side = unit-testable without the game.

## ✅ VERIFIED (offline layer): P6 DIPLOMACY DETERMINISTIC CORE (2026-07-12 ~20:30)

The campaign's highest-leverage gap (4 of 5 doc-11 scenarios) got its heart: `dashboard/gm_diplomacy.py` — pure-Python faction simulation per docs/P6_DIPLOMACY_DESIGN.md (sign-off doc written; Ken's three open questions at its tail). Schema (factions/relations/wars/treaties/rounds/banishments), score bands → states, passive drift, raid ground-truth feed (starts/updates wars with real casualties), legal-move menus computed from state (the LLM may ONLY pick from these), move consequences (war declarations shatter alliances and cascade ally scores; peace sizes reparations by loss ratio; tribute/trade/alliance treaties; 2-econ-pact cap), war fatigue (legal peace at 60, forced at 100), round-robin round ticker with injectable chooser + deterministic fallback.
**VALIDATION: test_diplomacy.py — 10/10 PASS offline** (negative paths included: illegal-move rejection, LLM-garbage fallback, treaty cap, no-duplicate wars, drift). Evidence: run output ALL GREEN, this date.
**Deliberately NOT done (deploy/dashboard-restart gated):** dashboard_server wiring (round scheduler thread, LLM chooser on the `diplomacy` critical lane, proclamations → P5 events API), mod-side faction roster/alignment feed, live validation. Dashboard restart would break Ken's live session UI — same gate as the mod deploy.
**Suggested commit title:** `feat(P6): AI diplomacy deterministic core + offline test suite (10/10); design doc for sign-off`

## ⚠ RECONCILIATION CORRECTION on P6 (AAR trigger — workflow violation, owned) (2026-07-12 ~20:45)

**gm_systems.py ALREADY CONTAINS a P6 substrate** (P3-P10 backends commit b994cff): faction_relations + diplomacy_log tables, get_relation, apply_diplomacy_action (war/peace/alliance/trade-pact caps/tribute), run_diplomacy_round (round counter, fatigue accrual, drift), _diplomacy_world_event (already feeds the P5 events pipeline!). My fresh gm_diplomacy.py DUPLICATES that substrate — I completed implementation before reconciliation had covered gm_systems.py (searched dashboard_server.py only; "search by resource, not feature name" violated: the resource was the DB tables).
**DISPOSITION:** gm_diplomacy.py is demoted to REFERENCE IMPLEMENTATION pending merge — its UNIQUE capabilities (absent from gm_systems) must be PORTED INTO gm_systems' existing save-scoped implementation: (1) legal-move MENUS computed from state — the bounded-LLM-choice architecture (the doc's core safety property), (2) alliance-shatter cascade with ally-score deltas, (3) reparations sized by loss ratio, (4) raid ground-truth feed (wars + real casualties), (5) round-robin mover selection, (6) banishment/pardon, (7) THE TEST SUITE (test_diplomacy.py 10/10 — port to drive gm_systems functions; offline validation of the MERGED implementation is the unit's exit gate).
**Merge = next unit, FRESH SESSION recommended** (gm_systems.py is shared production code; this session is deep in degradation territory — today's earlier mistakes all came from exactly this state). SPECIFIED, not started.
**AAR (triggered):** Sustain: offline-testable pure-module pattern + declared negative paths (the suite survives the merge). Improve: reconciliation checklist must enumerate ALL files owning the resource (DB tables) — grep the SCHEMA, not the feature name; I greped "faction" in one file and declared absence. Highest-risk weakness evidenced: implementing at high context depth skips workflow steps that were followed strictly earlier in the session — the fix is the fresh-session handoff discipline already in the operator protocol.

## ✅ VERIFIED (offline): P6 MERGE COMPLETE + GAME-TRUTH BRIDGE STAGED — scenario-1 pipeline closed at the dashboard layer (2026-07-12 ~21:15)

**Reconciliation redemption:** the gap matrix was WRONG in the right direction — gm_systems.py carries FULL P3-P10 substrates (orders parser with the doc's exact vocabulary, entities/visits/recruitment, events, diplomacy, romance stages/decay/initiative, death records with the 50-interaction gate + decline option, disease with seasonal odds/immunity, combat incidents) behind complete REST routes. THE REAL GAP WAS: nothing DRIVES them and the game never FEEDS them.
**Merged into gm_systems (duplicate gm_diplomacy.py retired):** legal-move menus (bounded LLM choice), alliance-shatter cascade w/ ally-score deltas, raid ground-truth feed w/ loss accounting + war escalation, loss-ratio reparations, round-robin agent move per round w/ injectable chooser + garbage fallback, **proclamation→propagation** (diplomacy events now reach every settler's known_events = dialogue context — the missing rumor link), new /api/diplomacy/raid route.
**VALIDATION: test_diplomacy.py 10/10 ALL GREEN** against the merged production module — including `test_proclamation_propagates_to_settlers`, which proves scenario 1's core loop end-to-end at the dashboard layer: war declared → world event → settlers hear it → known_events (what dialogue reads).
**GameTruthBridge STAGED (mod build code=0):** the mod becomes the heartbeat (diplomacy round every 20 real min; romance decay + disease season-tick hourly, real season via WorldTimeManager reflection) + death feed (DeathChronicler posts /api/death/record with cause BEFORE the LLM story — facts first). Wired: ColonyBuilder Phase("truth-bridge"), ResetSession.
**ACTIVATION GATE:** dashboard restart (gm_systems edits) + mod deploy — BOTH the same window (Ken's session end; exit sentinel armed). Live validation plan: diplomacy round logs in dashboard, a proclamation surfaces in a settler dialogue, death record row on next death, disease tick honors real season.
**Suggested commit title:** `feat(P6+bridge): diplomacy agent rounds w/ bounded menus + rumor propagation (10/10 offline); GameTruthBridge heartbeat + death feed`

## ✅ VERIFIED (offline): P9 DISEASE DEPTH — spread, seasonal onset, quarantine dampening, outbreak rumors (2026-07-12 ~21:40)

Doc 04's core, closed dashboard-side: **disease_spread** (每 contagious unquarantined case rolls SEASON_RISK vs every settler; quarantine cuts to 0.25x; infect()'s immunity + Medicine-skill resistance still apply), **seasonal_onset** (the season itself sickens — winter cold/fever, autumn cold/dysentery, etc.), both wired into /api/disease/tick (season param from GameTruthBridge's real WorldTimeManager read). **Outbreak events now PROPAGATE** to all settlers as rumor (scenario 2's "word travels" → dialogue context), matching the diplomacy proclamation loop.
**VALIDATION: test_disease.py 7/7 ALL GREEN** (seeded-RNG determinism): winter spread fires, quarantine < half of open spread over 40 seeds, summer < winter, seasonal onset fires, untreated plague → dead → REAL death_records row (P9→P8 integration), treated recovery grants immunity that blocks reinfection, outbreak event at 3 cases. Diplomacy suite regression: 10/10 still green. gm_systems parses clean.
**Scenario 2 pipeline status:** infection→spread→quarantine lever→death→death record→outbreak rumor→dialogue = COMPLETE at dashboard layer. Missing for live: mod feeds (visitor infection seeding, quarantine control surface, infirmary tier from real buildings) — deploy-window slices.
**Suggested commit title:** `feat(P9): disease spread + seasonal onset + outbreak rumor propagation (7/7 offline)`

## ✅ VERIFIED (offline): P7 ROMANCE AUTONOMY — bonds form by themselves; the village hears of weddings (2026-07-12 ~22:00)

Doc 06 closed at the dashboard layer: **romance_autonomous_tick** — pairs whose LIVE social state carries signal (relationships.romance+attraction from the mod's social hub) roll stage-appropriate courtship interactions (shared_meal→gift→courtship→kiss); initiative-trait settlers (sanguine/brave/reckless/proud) PROPOSE at courting+0.6 intimacy (the betrothal) and again at betrothed+0.8 (the vows); **betrothals & weddings become world events propagated to every settler** (scenario 4's social fabric — gossip reaches dialogue). Decay runs first: neglect cools untended bonds. New /api/romance/tick route; GameTruthBridge slow tick switched to it (staged).
**VALIDATION: test_romance.py 5/5 ALL GREEN** (seeded): no signal → no bonds; attracted pair → courting; initiative pair reaches betrothed AND married with the whole village hearing (world_event_knowledge = 4/4); dour+meek pair NEVER betroths across 200 seeds (initiative gate holds); 40-day neglect decays hard. Regressions: diplomacy 10/10, disease 7/7, gm_systems parses, mod staged build code=0.
**Suggested commit title:** `feat(P7): autonomous romance driver — courtship, initiative proposals, wedding rumors (5/5 offline)`

## ✅ VERIFIED (offline): P10 COMBAT DIRECTION — interventions, diplomacy feed, battle rumors (2026-07-12 ~22:20)

Doc 07 extended: **faction intervention** (any faction allied/strongly-friendly to the defender joins with a unique arrival line — scenario 3's "nearby lord intervenes"), **combat→diplomacy feed** (faction-vs-faction incidents ARE raids: report_raid sours relations, accrues war-stat losses, escalates to war past threshold — "casualties feed straight back into AI Diplomacy"), **battle rumor propagation** (incident world events reach every settler's dialogue context). Existing verdict machinery (aggressor/defender, militia-by-location, panic heuristic, companion stances by live trust, casualties→P8 death records) kept intact.
**VALIDATION: test_combat.py 5/5 ALL GREEN**: loyal companion stands with you / resentful one turns (trust 0.9 vs 0.1); gatehouse raid raises militia + panics civilians on casualties; allied Osric intervenes with arrival line; faction raid feeds diplomacy + records 2 deaths + all 3 settlers hear of the battle; a tavern brawl between settlers touches NO diplomacy. Full regression: diplomacy 10/10, disease 7/7, romance 5/5.
**TONIGHT'S ARC (4 systems deepened, 27 offline tests green):** P6 diplomacy (agent rounds, menus, cascades, rumor loop) · P9 disease (spread, seasons, outbreak rumors) · P7 romance (autonomous bonds, weddings the village hears) · P10 combat (interventions, diplomacy feed, battle rumors) — plus GameTruthBridge (staged) driving all of it from real game time and truth. The cross-system fabric of doc 11's scenarios now EXISTS at the dashboard layer: raids sour relations into wars, wars end in loss-priced reparations, proclamations/outbreaks/weddings/battles all reach settler dialogue as rumor.
**Suggested commit title:** `feat(P10): combat direction — faction intervention, diplomacy feed, battle rumors (5/5 offline; 27/27 suite)`

## DEPLOY DONE; auto-relaunch STOOD DOWN (machine-state conflict) (2026-07-12 ~19:10)

The staged-8 mod build IS DEPLOYED (in_sync=true). My pipeline auto-relaunched 2 min after Ken exited; the window came up minimized (160x40 — RESUME click void) and the process died shortly after — most plausibly KEN closed it (he had just quit deliberately; operator rule 2: never assume the machine is quiet). **No further launches tonight unless Ken launches or asks.** Validation of the 8 staged changes rides the next natural session — a passive observe-only sentinel watches for a Ken-launched game.
Parting gift from Ken's own 40-min session (old build, deployed fixes): ZERO freezes under live play; butchering_table BUILT with meat order queued; hunters armed, hunt=6; crisis plan cycling. The freeze class and the butcher link are live-validated by the player himself.
DASHBOARD RESTART still pending Ken's explicit OK (classifier-blocked, correctly) — activates tonight's P6/P7/P9/P10 upgrades (27/27 offline).

## ✅ VERIFIED (offline): P5 EVENT LIFECYCLE — events evolve, resolve on truth, expire (2026-07-12 ~19:25)

Doc 02's remaining core bullet closed: **events_evolve** — deterministic aging (active→evolving @2d → resolved @6d → expired @14d) with per-type narrative update notes appended to the update trail (dialogue can narrate "the fighting drags on..." vs "the matter is settled"); **war events resolve EARLY when their war actually ends** (state truth beats age — the world_events row watches faction_relations). New /api/events/evolve route; GameTruthBridge slow tick drives it (staged, build code=0).
**VALIDATION: test_events.py 3/3 ALL GREEN** (injected clock): young events untouched; full lifecycle with 3-entry update trail; war event resolves immediately on peace regardless of age. Full regression: diplomacy 10, disease 7, romance 5, combat 5 — **30/30 offline suite green.**
**MEANWHILE:** Ken relaunched himself at 19:05 → his live session is running the DEPLOYED staged-8 build; passive observer collecting validation evidence (shack/butcher/bridge lines).
**Suggested commit title:** `feat(P5): event lifecycle — age evolution, truth-driven war resolution, expiry (3/3; suite 30/30)`

## ✅ ACTUATOR CATALOG delivered (Ken's ask) (2026-07-12 ~19:35)

docs/ACTUATOR_CATALOG.md (synced to StarForge wiki, linked in _index): the complete validated action space — game-side reflection actuators (job priority, blueprints, zones, digs, production, research, equip, event answers, saves, speed, load rescue) each with real signature + preconditions + failure modes + validation date; the three standing hazards as law (singleton poisoning, map-scan freeze class, real-vs-game time); the dashboard REST layer (all P3-P10 routes incl. tonight's new ones) with the bounded-LLM hook per route; the propagation invariant ("if you add an event source, propagate it or nobody will ever speak of it"); the heartbeat map; LLM lane/budget rules with the mandatory loop-safety pattern. The knowledge triad is complete: player-competence-canon (WHAT) + how-things-work (game internals WHY) + actuator-catalog (HOW).

## STAGED (build code=0): REAL MOVEMENT ORDERS — doc 09's missing execution leg (2026-07-12 ~19:50)

Decompile ground truth: the game's order channel is `HumanoidInstance.EnemyBehaviour.CurrentOrder` (the slot EVERY FollowOrderBaseGoal<T> reads — drafted-move UI issues `MoveOrder(Vector3, guardModeRadius)` here; guard radius doubles as area-hold/patrol). New `MovementOrders` (TryMoveTo/TryRelease with honest INVOKED≠APPLIED logging) + OrderExecutor rewired: **move_to** = real MoveOrder (was an "explore" approximation), **patrol** = MoveOrder w/ guard r=10, **hold_position** = MoveOrder at own cell r=3 (was "rest"), **return_to_player** = come home (no player avatar exists), follow_player = honest unsupported. Target resolution: home/house/village/plaza keywords + "x,z" literals; unresolvable targets FAIL the step honestly instead of walking somewhere random.
**LIVE VALIDATION REQUIRED (next window):** whether FollowMoveOrderGoal WINS the GOAP race against work goals — watch a settler actually walk to an ordered cell (scenario 1's "send Mara to scout" slice). Also note: Ken's 19:02 relaunch never went world-live (menus only, exited 19:35) — staged-8 validation still pending too; observe-only posture holds.
**Suggested commit title:** `feat(P3): real movement orders via the game's MoveOrder channel (move/patrol/hold/return)`

## ACTIVATION COMPLETE (Ken authorized): dashboard restarted, new code LIVE; Steam wedge re-fixed (2026-07-12 ~20:10)

Two blockers cleared in sequence: (1) **"the game just closed" root-caused as the Steam wedge returning** — launches came up as hidden 160x40 windows and self-quit cleanly ~3 min in (orderly Unity teardown in Player.log = Steam handshake failure, Ken's own midday diagnosis); remedied per his banked ladder (Steam force-restart). (2) **Ken said "restart the dashboard"** — old python killed, start_dashboard.bat relaunched, **new routes verified live** (/api/romance/tick and /api/events/evolve answer 400-wants-save_id instead of 404). All five system upgrades (P5/P6/P7/P9/P10) + 30/30-tested code now SERVING. Game relaunched behind healthy Steam; definitive end-to-end validation watch in flight (staged-8 + bridge-v2 + MovementOrders mod × upgraded dashboard).

## ⚠ ROOT CAUSE CORRECTION [REPRODUCED]: "the game just closed" was MY OWN RESUME CLICK hitting the X (2026-07-12 ~20:20)

The three clean self-exits (~3 min post-boot) were NOT Steam: launches now come up as a COLLAPSED 160x40 window, the mod boots fine to the menu (log proves it), and my watch's RESUME click at relative (0.856, 0.328) of a 160x40 rect lands on the TITLE BAR'S CLOSE BUTTON. Player.log clean teardown + mod's "Harmony unpatched" shutdown line at click+seconds = the game was told to quit by my click. Ken's "the game just closed" report = me. The Steam restart was innocent-but-unnecessary (own the misdiagnosis — 2nd environment-blamed self-inflicted bug today; lesson repeated: suspect your own newest behavior first).
**FIX (in the cycle + MUST move into gm_devops later):** before ANY interaction, restore the window via user32 (SW_RESTORE → SW_MAXIMIZE → SetForegroundWindow), verify rect ≥800x600, and REFUSE to click a collapsed window. WHY launches collapse now (they didn't all afternoon) still unknown — suspect: window-state inheritance from the hidden dashboard context; diagnose separately. Backlog: dashboard /api/game/input must gate on window size (the same X-button trap exists for every fractional click).

## ✅ VERIFIED (LIVE): the big end-to-end run — 30-min stable, bridge heartbeat REAL, cross-village fix REAL, siteplan SUCCEEDS (2026-07-12 ~20:35)

Window-restore guard worked (1936x1096 restored from 160x40; the X-button trap is closed). WORLD LIVE 19:58:57 → SURVIVED 30 MIN. Live-validated tonight:
- **GameTruthBridge HEARTBEAT [LIVE]:** "diplomacy round requested" at 19:58:41 AND 20:19:34 (20-min cadence exact) + "slow tick: romance decay + disease tick (season=winter)" — REAL season read via reflection. DB proof: `diplomacy_log` rounds 1 & 2 for save Romsey on the NEW engine.
- **CROSS-VILLAGE CACHE FIX [LIVE]:** worldmap line is the NEW map's own scan (open 24% water 74% built 29, tower-cols 0, cellar-capable 42436) — Dolgellau's ghost stats are gone.
- **SITEPLAN NOW SUCCEEDS [LIVE]:** with real terrain data, "Jonatan Tilly chose (60,6,132) — flat 12x12 pad; close to home, near forest, deep cellar rock" + full leader-voice reasoning, and **home MOVED to the chosen site (shelter-first camp move fired)**. The prior "NO site fits" was the stale-cache bug all along.
- Zoner: refuse dump now d=21 (outside any house). House siting actively searching AT the leader's pad (radius 8/18 budgeted). No freezes; autosaves banking.
- **Rounds ran with 0 moves — CORRECT:** faction_relations is empty; the designed-but-unbuilt slice is the MOD-SIDE FACTION ROSTER FEED (P6 design doc listed it deploy-gated). Named next slice: /api/diplomacy/seed route + GameTruthBridge reflection read of FactionsController roster/alignment.
- **LIVE MOVEMENT TEST IN FLIGHT:** order 44 issued via /api/orders/issue — parser produced exactly [move_to(60,140), hold_position] (bounded, 0 unsupported). Observer watching for MovementOrders execution + the housing chain at the new site.

## FREEZE DETECTOR BUILT & DEPLOYED (Ken's ask: "the game is freezing constantly") (2026-07-12 ~20:55)

**FreezeDetector**: a real watchdog THREAD (immune to main-thread stalls) watches a per-frame heartbeat from Plugin.Update; any stall >2s is recorded with the phase running when the beat stopped, logged on recovery to mod log + validation/freeze_log.txt + a `freeze:` telemetry line. **Honest attribution built in:** ColonyBuilder's ~30 Phase() markers + recap-dismisser/event-pump/dialogue-ui markers report as `mod:<subsystem>`; the frame clears to `engine` after mod work — so every freeze names its side of the fence (our code vs the game's own hitches: autosave, meshgen, GC). Deployed (save banked first via SaveGuard flag — consumed ✔); 40-min live run in flight; freeze_log.txt is now the standing answer to "what froze".
Also live-validated this hour: order pipeline's NEGATIVE path (unknown settler id → honest per-step failure note) and a real settler-id order (45) reached MovementOrders and failed honestly with "no EnemyBehaviour (order slot)" → movement needs the DRAFT toggle first (banked: draft-then-order slice for the movement leg).

## FREEZE DETECTOR: FIRST BLOOD × 2 — both named freezers FIXED & deployed (2026-07-12 ~21:35)

The detector earned its keep in 3 minutes flat: (1) **`FROZE 8688ms during [mod:worldmap-scan]`** — the once-per-boot full-grid pass (678k nodes × ~6 reflected reads) held the main thread 8.7s; (2) **hard hang during [storage-place]** — TryPlaceStockpileNear's ~2500-query spiral, the same water-sim-race class, caught because the camp's move to the leader's site re-triggered STORAGE-AT-HOME. Detector gap fixed too: freeze START now logs immediately (recovery-only design missed the permanent hang).
**FIXES DEPLOYED:** worldmap scan moved to a DISPOSABLE BACKGROUND THREAD (pure C# model reads, no Unity API; stale-tear acceptable for strategy; wedge hangs the worker not the game; 180s timeout → retry) — slicing was rejected (8.7s at one 80ms slice per 12s tick = 22 min to sitplanner data). Storage spiral got the standard 50ms budget + radius-resume. 45-min validation watch in flight; freeze_log.txt is cumulative — Ken's "freezing constantly" now has a named, timestamped answer sheet.
**Suggested commit title:** `fix(freeze): watchdog-thread freeze detector with phase attribution; background worldmap scan; budgeted storage spiral`

## KEN'S AI PLAYBOOK integrated — binding map written, first rule implemented (2026-07-12 ~21:50)

Ken authored `F:\Downskies\going-medieval-ai-playbook.md` (IF/THEN decision rules, [V]/[A]/[?] evidence-tagged). **Its §9 "engine-binding gap" is largely already closed** — the actuator catalog IS that binding; wrote `docs/PLAYBOOK_BINDING.md` mapping every playbook rule → actuator with per-rule status (live/staged/specced/unbound). Knowledge stack is now a QUAD: canon (WHAT) + playbook (DECIDE) + how-things-work (WHY) + actuator-catalog (HOW), all linked in the wiki.
**Implemented immediately (map's priority gap #1): the 3-DAY FOOD WARNING TIER** — playbook §2 verbatim: stock < 12×3×settlers → all food jobs to prio 2, NOTHING suspended (research/art continue); telemetry census gains "⚠FOOD-LOW"; exits cleanly above buffer; crisis tier unchanged beneath it. Compiled code=0, STAGED (validation run in flight owns the current session).
Priority gaps queued from the map: draft-then-order (combat/movement), malnutrition+blood decompile reads (medical/combat rules), room-type formation probe, trade actuator.

## REVIEW caught my own fix's flaw: BUDGET GRANULARITY — per-origin checks deployed everywhere (2026-07-12 ~22:10)

Freeze-fix validation run: **worldmap background scan VERIFIED** (started→complete in 9s, zero main-thread cost — was 8.7s frozen). But the detector logged **12 freezes incl. a 67-SECOND one, all [mod:house-plan+beds]**: my 50ms time budget checked once per RADIUS RING — a radius-18 ring is ~39k map queries, and under water-sim contention one ring blew to 67s before the check ever ran. **Budget granularity was the flaw.** Fixed in EVERY spiral: house search + shack fallback + storage scan (per-ORIGIN checks), cellar scan (per-CELL), VillageLayout slot ring (50ms across slots). Deployed; 45-min validation with freeze-count verdict in flight — the freeze log is now a before/after scoreboard (12+1 freezes on per-ring builds; target: engine-only entries).
Also carried in this deploy: FOOD WARNING tier (playbook §2), detector freeze-START logging.
**Suggested commit title:** `fix(freeze): per-origin time-budget checks in all map-scan spirals (67s ring freeze); food warning tier; detector ongoing-freeze logging`

## FREEZE ENDGAME DEPLOYED: WorldSense threaded + SNAPSHOT-DRIVEN SITING (2026-07-12 ~22:40)

Validation verdict on per-origin budgets: 40-min stable, freezes 13→7, **FOOD WARNING tier fired live** (playbook rule validated), cookfire placed. Remaining culprits named by the detector: (1) **[mod:worldsense] 80,844ms** — the home-region raster was WorldMap's unthreaded twin → moved to a disposable background worker (same pattern); (2) the budget-starvation paradox: with contention pricing each live query in SECONDS, budgeted spirals advanced ~1 origin/tick → full passes take hours → the shack fallback (fires only after a complete failed pass) was unreachable → beds still 0.
**THE ENDGAME (banked escalation path, now deployed): SNAPSHOT-DRIVEN SITING** — new WorldMap.SnapshotBuildableDry(x,y,z): pure array reads from the completed background scan (water/built/tree/level checks); FALSE skips the cell with ZERO live queries; TRUE proceeds to live CanPlace/CellIsDry verification. Wired as the FIRST check in the house spiral, shack fallback, and village slots. On a 74%-water map, ~¾ of candidate cells now cost nanoseconds instead of seconds — full siting passes become feasible again, which un-starves the shack fallback → BEDS.
50-min validation in flight (freeze count + the beds quest). 
**Suggested commit title:** `fix(freeze): background WorldSense raster; snapshot-driven siting pre-filter (freeze-class endgame)`

## NATIVE CRASH ROOT-CAUSED [REPRODUCED]: ScheduleRouter's substring mapper scheduled RoleJob into Work hours (2026-07-12 ~23:00)

Crash_2026-07-13_023620596 (45s after world-live): Player.log spun "CURRENT_HOUR None GCONT: 0" hundreds/sec for Girard (role-less) + Margaria, then a native crash. Root cause in ScheduleRouter.HourVal: the substring chain `Contains("work")||Contains("job")` matched **"Working" then OVERWROTE it with "RoleJob"** (enum order) — every Work hour (9h/day) was written as RoleJob; role-less settlers resolve RoleJob to HourType.None(-1) → goal-selection crash-loop → native crash. Latent all evening; only bit when a resume landed in Work hours for a role-less settler. ALSO: the mapper could theoretically write None itself ("none" was in the Anything matcher).
**FIX DEPLOYED:** None is never schedulable (skipped outright); role-ish names consumed BEFORE the work branch; first-match-wins (no overwrites). Persisted-broken schedules self-heal: ApplyAll re-applies each session (the _done gate resets on load). 55-min validation in flight (crash recurrence + CURRENT_HOUR None count + the beds quest under snapshot siting).
**Suggested commit title:** `fix(schedule): HourType mapper wrote RoleJob into Work hours — native crash for role-less settlers; None never schedulable`

## CRASH ROUND 2: the SAVE is poisoned; sanitizer moved to per-second cadence (2026-07-12 ~23:15)

Second crash 3.5 min post-boot with **1,898 CURRENT_HOUR None lines in ONE boot** — the mapper fix was correct but LOST THE RACE: Romsey's persisted schedules carry RoleJob Work-hours from the buggy build, the goal loop spins from LOAD-IN, and ColonyBuilder's 12s tick re-applies too late. **FIX DEPLOYED: ScheduleRouter.ApplyAll now runs from ProcessNPCs (per-second) the moment settlers validate** — the sanitizer beats the crash window (spam ran 45s-3min before death). Also seen pre-crash: 12.2s house-plan+beds freeze (snapshot prefilter underperforming or pre-spiral setup unbudgeted — investigate after stability) + camp_fire blueprint UNREACHABLE (the reachability slice's case grows).
60-min validation in flight: None-count (target ~0 after sanitize), crash recurrence, freezes, beds quest.

## ⚠ MAJOR REVERT [REPRODUCED-correlation]: off-thread game-state reads were CRASHING the game (2026-07-12 ~23:30)

Three native crashes (22:36, 22:45, 22:49 — Crash_ dumps) + a 1000x map-query slowdown (2.5s/query → the 12-17s house-plan freezes) all began EXACTLY with the background-scan builds. Enumerating live game collections (GridSpaceData) from worker threads while the main thread mutates them = the classic Unity/Mono native-crash recipe. My "pure C# model data is thread-safe to read" assumption was WRONG — it's the game's mutating state, full stop.
**REVERTED + REPLACED:** WorldMap.Scan → main-thread SLICED enumerator state machine (150ms/tick, kept enumerator, pass 1→2→summary; full pass over ~a minute of ticks); WorldSense raster → DEFERRED ENTIRELY (honest no-op; nothing critical consumes it; slice it before re-enabling). ScheduleRouter sanitizer (per-second) rides along.
**LAW BANKED (catalog + wiki update pending): NO game-state reads off the main thread, EVER.** The freeze-vs-crash tradeoff is resolved by SLICING, not threading.
Eyes-on gate added to the watch (Ken's directive): post-click screenshot saved every cycle. 60-min validation in flight: crash recurrence, None-count, freeze log, sliced-scan completion, beds.

## STAND-DOWN + PASSIVE POSTURE (2026-07-13 ~00:10) — Ken at the machine; two of MY interference bugs owned

Eyes-on gate (Ken's directive) caught both: (1) post-click screenshot showed the MAIN MENU — RESUME clicks were silently failing because **pydirectinput cannot click negative-coordinate monitors** (game window at x=-1928; every intermittent click failure today, including the X-button kills, traces here); (2) my fix (move window to primary) MOVED THE GAME ONTO KEN'S WORKING MONITOR mid-task, and the "verification screenshot" captured HIS CHROME (the dashboard screen endpoint matches window titles — a Chrome tab named "Going Medieval" qualifies — eyes-on evidence only valid when the game is frontmost). ALL automation frozen; passive file-only sentinel armed (2h).
**Validated before stand-down: CURRENT_HOUR None = 0** (crash chain broken: mapper fix + per-second sanitizer). Open dashboard bugs filed: screen endpoint title-match trap; input endpoint needs window-size gate + primary-monitor handling (or SendMessage-direct clicks).

## ✅ WORLD LIVE, PIXEL-VERIFIED — the click chain is fully solved (2026-07-13 ~00:25)

Ken cleared the stand-down ("carry on"). Eyes-on gate caught the LAST click bug: even on the primary monitor, the window at x=376 put the RESUME button at screen-x 2033 — PAST the primary's right edge. **The working recipe (now doctrine for every cycle): SetWindowPos to (0,0) → verify button lands on-screen → click → screenshot-verify.** Result: Romsey world-live, day 10, all four settlers on screen. Alerts visible: settlers STARVING + wounds need tending (crisis threshold pop*12 should be routing; watch confirms). 45-min validation watch armed (None-count, freezes, sliced scan, pillars, beds).
Dashboard backlog (recorded): input endpoint should do the move-to-primary+bounds-check itself; screen endpoint must match the game PROCESS's window, not any window titled "Going Medieval" (Chrome-tab trap).

## THE NO-HOUSE ROOT CAUSE [REPRODUCED in log]: radius-2 infinite loop — SNAPSHOT-FIRST siting deployed (2026-07-13 ~00:40)

Ken (rightly): "a lot of words for a village without a house." The log had the receipt: "site search begin (radius 2..18)" EVERY TICK — when the 50ms budget expires MID-RING, resume saves the ring but not the position inside it; a ring that can't finish in one budget NEVER advances. The search has been spinning at radius 2 for hours = no full pass = the shack fallback (which requires a COMPLETED failed pass) never fired = beds 0/4 forever.
**FIX DEPLOYED: SNAPSHOT-FIRST SITING** — the house search now WAITS for the sliced worldmap scan (~1 min/session), then the snapshot prefilter makes candidates near-free and the full pass completes in a tick or two → either the longhouse sites at the leader's pad or the shack fallback finally triggers. Also: per-pass (not per-slice) search logging; deploy-order goof owned (build-while-running → copy failed; kill→deploy→launch enforced).
50-min watch armed: THE HOUSE QUEST is the headline verdict.

## KEN'S PIVOT: VillageForge — offline coherent-village generator, v1 SHIPPED (2026-07-13 ~01:00)

Ken (with reference screenshots of real player towns): "I don't want a fallback basic house... design some kind of system OUT of the game that can generate random coherent villages under the same context as in the game, then port it over — input mimics the game situations, output we can see and validate."
**DONE THIS HOUR:** (1) starter-shack fallback DISABLED (Ken's explicit word — the system reasons toward the endgame, not toward hovels); (2) `tools/villageforge/forge.py` v1: Situation input (seed/stage 1-4/pop/biome marsh-valley-hillside) → synthetic terrain → site-scored center → plaza+street skeleton (laid at stage 1 so stage 4's walls make sense — "reasoning around the endgame") → varied houses on street-adjacent lots + great hall/church/graveyard/workshop/butcher/barn/fields/stockpile yard by stage → stage-4 wall ring w/ corner towers + gate, water gaps as moat → **village.png (the judgeable artifact) + plan.json (mod-actuator vocabulary: wood_wall_element/door/floor + rects — the port format)**. Four samples delivered to Ken (marsh st1/st2, valley st3/st4).
**v1 flaws visible on sight (the loop working as designed):** wall hull crosses the river / isolated house across the river inside the wall (needs road-connected-component coherence: everything in the wall must reach the plaza without crossing water); houses scattered near roads instead of ALONG street edges in rows (reference look); wall is a plain bounding box (should hug the settlement organically). v2 iteration = exactly these three.
**Port path (specced):** mod exports real Situation (WorldMap snapshot arrays + roster) → forge generates plan.json → OrderExecutor/HouseBuilder executes ordered blueprints (machinery proven). LLM hook: the architect's role becomes choosing forge PARAMETERS + naming buildings, not inventing geometry.

## VillageForge v2: PHASES + FORWARD PLANNING + 3D (Ken's directive) — shipped, iterating (2026-07-13 ~01:30)

Ken: "add a phase system... needs now and needs later, more people will show up... it must also think in 3D — cellars, more than 3 floors." **v2's central mechanism: the ENDGAME master plan is generated FIRST, each phase builds a SUBSET** — phase 1's cellar is already under phase 3's great hall; the founding street grid is the same one the final walls enclose; unbuilt lots render as ghost outlines (RESERVED). Phases follow the reference guide's staged recommendations verbatim (founding: shelter+research+cold-cellar+crops+chokepoint stub → establishment: library/workshop/hall/pens → fortress: full walls+towers+keep+church+graveyard+infirmary+market → endgame district). **3D: every building carries floors(1-3) + cellar depth(0-2); cellars fully walled, beams every 3 tiles, stair shafts; renders show EVERY level as panels (L-2/L-1/surface/F2/F3)**; plan.json per phase = only that phase's new work, with cellar/upper-floor sub-specs (dig_levels, beam_every, wood_beam, wood_stair_straight — actuator vocabulary).
v2.0→v2.1 fixes from LOOKING (the loop works): margin-included-road rejected every lot (built 0/0) → footprint-only collision; river-crossing hull fixed by same-landmass coherence rule. **Visible remaining (v2.2):** largest buildings (10x8 great hall) sometimes fail placement in tight rings → lot-first sizing; when the hall misses, L-2 stays empty; want row-alignment along street frontage tightened. 8 phase-sequence images delivered to Ken.

## GOAL SET (Ken /goal 2026-07-13 ~02:00): THE CHRONICLE TEST — three cumulative gates, one run, Ken's eyeballs as the final oracle

Gate 1 Coherent Colony (3 in-game days hands-off: beds by night 2 via a VillageForge phase-1 plan, food ≥1-day buffer, a cellar level dug, freeze-log clean) · Gate 2 Living World (diplomacy round with REAL factions + one event propagated into a settler's dialogue, DB-proven) · Gate 3 The Scenario (one doc-11 slice end-to-end: game event → world event → rumor in dialogue → consequence; Ken reads the transcript and believes it).

**CAMPAIGN PLAN — the two solder-wires first:**
- **UNIT A (real-terrain bridge, small):** after the sliced scan completes, the mod EXPORTS the snapshot arrays (Surface/Cls per column) to validation/worldmap_grid.txt; VillageForge gains --from-game import replacing synthetic terrain → plans are generated against the REAL map at the leader's site. [mod C# + forge py, offline-testable]
- **UNIT B (faction roster feed, small):** GameTruthBridge reads FactionsController roster+player alignment via reflection → POST new /api/diplomacy/seed (ensure_entity kind=faction + initial relation from alignment). Rounds start making REAL moves; proclamations flow. [dashboard py + mod C#; dashboard restart at next window]
- **UNIT C (PlanExecutor, the big one):** mod-side executor consuming plan.json phase work-items → existing actuators (batched blueprints for walls/doors/floors per rect, cellar digs via DigMarker path, fields via FarmPlanner, zones). Idempotent (BuildingExistsAt skip), budgeted, persisted progress. This REPLACES HouseBuilder's improvised siting as the construction path (goal: no improvised placement).
Sequence: A+B staged now → deploy window (machine-state checked) → validate → C → Gate 1 attempt on a fresh save.

## ✅ UNITS A+B BUILT (both Chronicle solder-wires, staged) (2026-07-13 ~02:20)

**UNIT B — faction roster feed [Gate 2's key]:** GameTruthBridge.SeedFactionsOnce reads the REAL roster via reflection (`WorldMap.Data.FactionInstances` → NameLocalized + PlayerFriendliness — decompile-grounded, api_index 33315/35499/49954), posts to NEW /api/diplomacy/seed which seeds faction entities + player↔faction relations (friendliness 0..100 → relation -1..1, war below -0.6) and logs a "known powers of the region" proclamation. Runs before the first diplomacy round (roster before rounds). Reset-safe.
**UNIT A — real-terrain bridge [Gate 1's foundation]:** WorldMap.DumpGrid exports the completed snapshot at FULL resolution (class chars + surface hex) to validation/worldmap_grid.txt; forge2 --from-game consumes it — master plans generated against the REAL map ('#' built = never build there).
Mod staged build code=0; gm_systems parses; **full offline regression 30/30 GREEN across all five suites.** Activation: mod deploy + dashboard restart at next machine-free window. NEXT: Unit C (PlanExecutor — forge plan.json → in-game blueprints), then the Gate 1 fresh-save attempt.

## A+B ACTIVATED: deploy + dashboard bounce complete; activation watch in flight (2026-07-13 ~02:40)
